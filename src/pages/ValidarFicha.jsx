...
export default function ValidarFicha() {
  ...
  useEffect(() => {
    fetch('/api/validar-ficha?token=' + encodeURIComponent(token))
  ...
  async function validar() {
    ...
    try {
      const r = await fetch('/api/validar-ficha?token=' + encodeURIComponent(token), {
          ...
          <div style={{ padding: '26px 28px 28px', textAlign: 'center' }}>
            {ya ? (
              <>
                ...
                <h2 style={{ margin: '0 0 6px' }}>Ficha validada</h2>
                <p style={{ color: C.cuerpo }}>La ficha queda firmada y validada por ambas partes. El alcance del agente de <b>{d.cliente_nombre}</b> queda <b>aprobado</b>.</p>
              </>
            ) : (
              <>
                <h2 style={{ margin: '0 0 4px' }}>Valida la ficha</h2>
                <div style={{ height: 3, width: 44, background: C.teal, borderRadius: 3, margin: '10px auto 16px' }} />
                <p style={{ color: C.cuerpo, margin: '0 0 8px' }}><b>{d.firmante_nombre || d.cliente_nombre}</b> ha firmado <b>{d.titulo}</b>.</p>
                <p style={{ color: C.tenue, fontSize: 13.5, margin: '0 0 16px' }}>Introduce el código que te enviamos por email para validarla como agencia y dar por aprobado el alcance.</p>
                ...
                <button onClick={validar} disabled={enviando}
                  ...
                  {enviando ? 'Validando…' : '✔ Validar la ficha'}
                </button>
